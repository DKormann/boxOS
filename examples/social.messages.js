...
return ctx.atomic(function messages(tx) {
  ...
  if (input.action == "send") {
    if (typeof input.target != "string" || input.target == input.account || typeof input.text != "string" || input.text.length < 1 || input.text.length > 2000) throw "Invalid message";
    ...
    thread.push({ id: input.id, from: input.account, text: input.text }); tx.state.private.set(threadKey, thread);
    let found = false; let i = 0;
    while (i < conversations.length) { if (conversations[Number(i)] == input.target) found = true; i = i + 1; }
    if (!found) { conversations.push(input.target); tx.state.private.set(listKey, conversations); }
    let otherKey = "conversations:" + input.target; let other = tx.state.private.get(otherKey) || []; found = false; i = 0;
    while (i < other.length) { if (other[Number(i)] == input.account) found = true; i = i + 1; }
    if (!found) { other.push(input.account); tx.state.private.set(otherKey, other); }
