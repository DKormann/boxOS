...
export class BoxMethodSyntaxError extends SyntaxError {
...
class Parser {
  ...
  private functionExpression(): void {
  ...
  private functionTail(localNames: readonly string[] = []): void {
    this.expect("(");
    ...
    if (this.match("{")) {
      ...
      if (!this.at("}")) {
        do {
          ...
          if (keys.has(key.value)) this.fail(`Duplicate object property '${key.value}'`, key);
          keys.add(key.value);
  ...
  private expectPropertyIdentifier(message: string): Token {
    if (this.current.kind !== "identifier" && this.current.kind !== "keyword") this.fail(message);
    const token = this.current;
    this.advance();
    return token;
  }
...
/**
 * Validate a method body against boxOS's deliberately small JavaScript subset.
 * The only names initially in scope are `argumentNames`; all others must be declared locally.
 * Throws BoxMethodSyntaxError when the source is outside the subset.
 */
const METHOD_BINDINGS = ["ctx", "input", "JSON", "Math", "String", "Number"] as const;
const CALLBACK_BINDINGS = ["ctx", "JSON", "Math", "String", "Number"] as const;

function analyzeCode(source: string, argumentNames: readonly string[], description: string): { references: string[] } {
  if (typeof source !== "string") throw new TypeError(`${description} source must be a string`);
...
/** Parse and validate a box method body. */
export function analyzeMethodCode(source: string): { references: string[] } {
  return analyzeCode(source, METHOD_BINDINGS, "Method");
}

export function validateMethodCode(source: string): void {
  analyzeMethodCode(source);
}

export function isValidMethodCode(source: string): boolean {
  try {
...
/**
 * Parse a callback serialized with Function.prototype.toString. Wrapping the
 * source makes the function a standalone expression; no initiating-method
 * locals are in scope, so attempted closure capture is rejected.
 */
export function validateCallbackCode(source: string): void {
  analyzeCode(`(${source});`, CALLBACK_BINDINGS, "Callback");
}

export function isValidCallbackCode(source: string): boolean {
  try {
