...
class Parser {
  ...
  private unary(): Expression {
    if (this.matchKeyword("await")) {
      if (!this.allowAwait || this.functionDepth !== 0) this.fail("await is available only at method top level", this.previous);
      this.unary();
      return VALUE;
    }
 ...
 */
export function analyzeMethodCode(
  source: string,
  argumentNames: readonly string[] = ["ctx", "input", "JSON", "Math", "String", "Number"],
  allowAwait = false,
): { references: string[] } {
  if (typeof source !== "string") throw new TypeError("Method source must be a string");
  if (new TextEncoder().encode(source).length > 64 * 1024) throw new TypeError("Method source exceeds 64 KiB");
  const parser = new Parser(source, argumentNames, allowAwait);
  parser.parse();
  return { references: [...parser.references] };
}

export function validateMethodCode(
  source: string,
  argumentNames: readonly string[] = ["ctx", "input", "JSON", "Math", "String", "Number"],
  allowAwait = false,
): void {
  analyzeMethodCode(source, argumentNames, allowAwait);
}

export function isValidMethodCode(
  source: string,
  argumentNames: readonly string[] = ["ctx", "input", "JSON", "Math", "String", "Number"],
): boolean {
  try {
    validateMethodCode(source, argumentNames);
    return true;
  } catch {
    return false;
  }
